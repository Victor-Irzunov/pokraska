...
const Icon = () => {
  const viberNumber = `+${phoneNumbers.phoneSocial1Link}`;
  const waNumber = phoneNumbers.phoneSocial1Link; // без +
  const tgHandle = phoneNumbers.phone1Link; // username или номер

  return (
    <div>
      <nav aria-label="Социальные мессенджеры">
        <ul className="flex items-center gap-2.5">
          <li>
            <a
              href={`viber://chat?number=%2B${phoneNumbers.phoneSocial1Link}`}
              target="_blank"
              rel="noopener noreferrer"
              className="mx-2.5 lg:tooltip text-primary focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary rounded-full"
              data-tip="Viber"
              aria-label={`Открыть чат в Viber: ${viberNumber}`}
              title={`Viber: ${viberNumber}`}
            >
              <span className="sr-only">{`Открыть чат в Viber: ${viberNumber}`}</span>
              <svg
          ...
          <li>
            <a
              href={`https://wa.me/${waNumber}?text=${encodeURIComponent("Нужна консультация")}`}
              target="_blank"
              rel="noopener noreferrer"
              className="mx-2.5 lg:tooltip text-primary focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary rounded-full"
              data-tip="WhatsApp"
              aria-label={`Написать в WhatsApp: +${waNumber}`}
              title={`WhatsApp: +${waNumber}`}
            >
              <span className="sr-only">{`Написать в WhatsApp: +${waNumber}`}</span>
              <svg
                viewBox="0 0 1024 1024"
                xmlns="http://www.w3.org/2000/svg"
                className="fill-current"
          ...
          <li>
            <a
              ...
              className="mx-2.5 lg:tooltip text-primary focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary rounded-full"
              data-tip="Telegram"
              aria-label={`Перейти в Telegram: ${tgHandle}`}
              title={`Telegram: ${tgHandle}`}
            >
              <span className="sr-only">{`Перейти в Telegram: ${tgHandle}`}</span>
              <svg
                viewBox="0 0 1024 1024"
                xmlns="http://www.w3.org/2000/svg"
                className="fill-current"
